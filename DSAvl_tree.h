...
class DSAvl_tree{    
    private:
        ...
        struct AvlNode{
            ...
            // with a value
            AvlNode(const Comparable &theKey, Comparable2 &theValue, AvlNode *lt, AvlNode *rt, int h)
            : key{theKey}, left{lt}, right{rt}, height{h} {
    ...
    */
   vector<Comparable> getKeys(){
        vector<Comparable> theVector;
        getKeys(root, theVector);
        return theVector;
   }

    /**
     * Returns a vector of all values for a key
    */
   vector<Comparable2> getValues(Comparable theKey){
        vector<Comparable2> theVector;
        set<Comparable2>* theSet;
        getValues(root, theKey, theSet);
    ...
    /**
     * Inserts y value into the tree on x node. IMPLEMENTED BY ME
    */
...
private:
    /**
     * Internal method to insert comparable into a subtree.
     * x is the comparable item to insert.
    ...
    AvlNode *clone(AvlNode *t) const
    {
        ...
        return new AvlNode{t->key, t->values, clone(t->left), clone(t->right), t->height};
    }
    ...
    /**
     * This uses LNR to return a vector containing all the keys
    */
   void getKeys(AvlNode* node, vector<Comparable>& theVector)
   {    
        // if the node is a nullptr just return
        if(node == nullptr){
            return;
        }

        // L
        getKeys(node->left, theVector);
        // N
        theVector.push_back(node->key);
        // R
        getKeys(node->right, theVector);
   }

   void getValues(AvlNode* node, Comparable theKey, set<Comparable2> *&theSet){
        if (node == nullptr){
            return;
        }

        if (node->key == theKey){
            theSet = &node->values; // Key found, writing theSet is now the same as the key node's value field
        }
        else if (theKey < node->key){
            getValues(node->left, theKey, theSet);
        }
        else{
            getValues(node->right, theKey, theSet);
        }
   }
